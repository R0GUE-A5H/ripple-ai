...
AI_METADATA = {
    ...
    "staging_trips": {
        "dataset_role": "staging",
        "timestamp_column": "trip_date",
        "sla_days": "1",
        "transformation_logic": (
            "Create cleaned staging data from raw_trips. "
            "Filter invalid rows. "
            "Create trip_date, trip_duration_min and pipeline_status. "
            "Do not aggregate."
        ),
    },
    "mart_daily_summary": {
        ...
        "sla_days": "1",
        "transformation_logic": (
            "Aggregate staging_trips by trip_date. "
            "Compute trip_count, total_fare and avg_passenger_count."
        ),
    },
    "v_staging_from_raw": {
        "dataset_role": "view",
...
def attach_custom_properties(emitter, urn_map):
    count = 0

    for dataset, props in AI_METADATA.items():
        if dataset not in urn_map:
            continue

        emitter.emit(
            MetadataChangeProposalWrapper(
                entityUrn=urn_map[dataset],
                aspect=DatasetPropertiesClass(customProperties=props),
            )
        )

        print(f"    ✓ custom_properties → {dataset}")
        count += 1
...
def main():
    ...
    for instance in instances:
        ...
        print(f"\n    Adding AI metadata...")
        attach_custom_properties(emitter, urn_map)
